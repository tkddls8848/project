...
번들로 바꾸는 단계는 [service-factory-plan.md](service-factory-plan.md)에 있다.

```text
[A단계] 시나리오 설계 (멀티에이전트)          [B단계] 웹 서비스 생성 (결정형 9노드)
질의 -> 문서 선택 -> 시나리오 초안       ->    service-factory-plan.md
     -> 결정형 바인딩 -> 결정형 검증
     -> ScenarioDoc
```
...
버튼은 [service-factory-plan.md](service-factory-plan.md) 소관이다.
