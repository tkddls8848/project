...
DEFAULT_STORAGE_DIR = PROJECT_ROOT / "api_storage"
...
HERMES_ENV_DEFAULTS = {
    ...
    # earlier tool result, so this budget is what bounds token spend per run.
    # MAX_HERMES_TOOL_CALLS is the hard ceiling this may be raised to.
...
# CLAUDE.md의 Nara MCP 규약이 허용하는 최대 호출 횟수다. 기본값은 4지만
# llama-3.1-8b가 재검색을 한 번 더 하는 경우가 있어 6까지 허용한다.
MAX_HERMES_TOOL_CALLS = 6
...
def _hermes_max_tool_calls() -> int:
    raw_value = _hermes_env("NARA_HERMES_MAX_TOOL_CALLS")
    try:
        return int(raw_value)
    except ValueError as exc:
        raise ValueError(
            "NARA_HERMES_MAX_TOOL_CALLS는 "
            f"1에서 {MAX_HERMES_TOOL_CALLS} 사이 정수여야 합니다: "
            f"{raw_value!r}"
        ) from exc
...
class Settings:
    ...
    hermes_max_tool_calls: int = field(default_factory=_hermes_max_tool_calls)
    ...
    def __post_init__(self) -> None:
        value = self.hermes_max_tool_calls
        if (
            isinstance(value, bool)
            or not isinstance(value, int)
            or not 1 <= value <= MAX_HERMES_TOOL_CALLS
        ):
            raise ValueError(
                "hermes_max_tool_calls는 "
                f"1에서 {MAX_HERMES_TOOL_CALLS} 사이 정수여야 합니다 "
                f"(Nara MCP 호출 상한): {value!r}"
            )
