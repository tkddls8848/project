...
from app.agent import AgentRunManager, _Run
from app.config import Settings
from app.hermes_client import HermesRunResult
from app.nara_client import NaraServiceError
from app.result_materializer import materialize_design_result
...
def test_stream_does_not_lose_terminal_event_emitted_while_arming_wait():
    async def scenario():
        manager = AgentRunManager(Settings(critic_mode="disabled"))
        run = _Run(
            run_id="race-window",
            request=AgentRunRequest(query="race window"),
        )
        manager._runs[run.run_id] = run

        class EventArrivingDuringClear(asyncio.Event):
            emitted = False

            def clear(self):
                if not self.emitted:
                    self.emitted = True
                    manager._emit(run, "completed", "completed", "종료 이벤트")
                    run.done.set()
                super().clear()

            async def wait(self):
                raise AssertionError("종료 이벤트 발행 후 wait하면 신호를 잃은 것입니다.")

        run.changed = EventArrivingDuringClear()

        events = [event async for event in manager.stream(run.run_id)]

        assert [(event.sequence, event.name) for event in events] == [(1, "completed")]

    asyncio.run(scenario())


def test_failed_tool_completion_message_reports_failure():
    async def scenario():
        manager = AgentRunManager(Settings(critic_mode="disabled"))
        run = _Run(
            run_id="failed-tool",
            request=AgentRunRequest(query="failed tool"),
        )

        await manager._on_hermes_event(
            run,
            {
                "event": "tool.completed",
                "tool": "mcp__nara__get_api_detail",
                "error": True,
            },
        )

        assert run.events[-1].status == "failed"
        assert "실패했습니다" in run.events[-1].message
        assert "완료했습니다" not in run.events[-1].message
