...
def test_naive_index_timestamp_is_compared_instead_of_crashing(tmp_path):
    # 매니페스트는 KST offset을 갖지만 NARA_INDEX_BUILT_AT은 손으로 넣는 값이라
    # offset이 빠지기 쉽다. 예전에는 여기서 TypeError가 나 검사 전체가 죽었다.
    write_manifest(tmp_path, "before.json", "2026-07-01T00:00:00+09:00", "sha256:before")
    write_manifest(tmp_path, "after.json", "2026-07-03T00:00:00+09:00", "sha256:after")

    report = check_document_freshness(
        ["openapi_new:15000827"], tmp_path, "2026-07-02T00:00:00"
    )
