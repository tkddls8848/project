...
- 적용 프로젝트: `C:\project\apps\hermes_poc`
- 보호 대상: `C:\project\apps\workbench` 변경 금지
- 파생 문서
...
| 문서 신선도 | 원안에 없음 | `check_doc_freshness` 도구 + freshness 단계 | 추가 구현 |
...
app/freshness.py      크롤러 매니페스트 기반 문서 신선도 검사
...
POST /agent/design-runs (202, run_id 즉시 반환)
  └─ asyncio task
       ...
       ├─ [freshness] 크롤러 매니페스트 대조 (인프로세스, 프로브 아님) → DocumentFreshness[]
...
원안의 네 도구에 신선도 도구가 추가되어 **현재 노출 도구는 다섯 개**다.
...
| `check_doc_freshness` | 선택 문서 신선도 확인 (읽기 전용) | 1 (배치) | 프로브 호출 없음 (§10.2) |

현재는 Python이 호출 횟수를 구조적으로 결정하므로 **한 run의 프로브는 최대
6회**이며 별도 카운터가 필요 없다. `check_doc_freshness`는 PoC 내부에서 직접
실행되므로 프로브 예산에 들어가지 않는다. 모델이 도구를 선택하게 되면 원안
상한과 12회 총량 제한을 실제 카운터로 강제해야 한다.
...
`config/hermes.example.yaml`이 Nara MCP만 등록하고 `tools.include`로 위 다섯
도구를 화이트리스트하며 `resources`·`prompts`를 끈다. `memory.write_approval`과
`skills.write_approval`은 `true`다. 신선도 도구가 쓰는 `NARA_STORAGE_DIR`·
`NARA_INDEX_BUILT_AT`도 MCP subprocess env에 명시한다.
...
freshness · critic · completed · failed · cancelled`
...
메인 루프 종료 후 critic 앞에서 1회, 선택 문서의 크롤러 매니페스트
(`nara_storage/manifests/*.json`)와 활성 인덱스 빌드 시각을 대조한다.
`app/freshness.py`는 파일을 읽기만 하며 **재크롤·인덱스 재빌드를 트리거하지
않는다** (탐구 문서 후보 C의 범위 제한).

- 판정: `fresh`(인덱스 이후 체크섬 변경 없음) · `stale`(변경 감지) ·
  `unverified`(인덱스 빌드 시각 미설정, 매니페스트 항목 없음, `service_id`
  형식 불일치, 인덱스 이전 기준 매니페스트 없음)
- `NARA_INDEX_BUILT_AT`가 비어 있으면 전부 `unverified`다. **없는 메타데이터로
  신선도를 주장하지 않는다**는 것이 이 설계의 핵심이다.
- `fresh`가 아닌 항목은 결과 `warnings`에 추가되고 `AgentRunResponse.freshness`로
  노출된다. 실패해도 run은 `completed`를 유지한다 (fail-soft).
- `NARA_DOC_FRESHNESS_MODE=disabled`로 끌 수 있다.

`stale`은 향후 `insufficient_evidence` 판정의 새로운 사유("문서가 오래됨")로
연결될 수 있다. 현재는 경고까지만 한다.
...
- 완료: `/design`은 원형 그대로 유지되고 별도 경로로 에이전트가 추가됐다.
  테스트는 10개에서 **40개**로 늘었고 전부 통과한다. Workbench 파일 변경 없음.
...
| `NARA_INDEX_BUILT_AT` | (빈 값) | 활성 인덱스 빌드 시각(ISO 8601). 비면 전부 `unverified` |
...
| `GET` | `/agent/design-runs/{id}/flow` | 구현 (§10.3) |
...
- 남음: 기준선/에이전트 모드 전환, 검색어 변경 이력, 선택/제외 API 분리 표시,
  ...
  (`run_id`가 메모리에만 있다), 승인 UI, 신선도 결과 전용 표시
  (현재는 타임라인 라벨과 경고 문자열로만 노출된다).
...
현재 40개 테스트가 외부 서비스 없이 통과한다.

| 파일 | 개수 | 범위 |
|---|---:|---|
| `test_agent.py` | 5 | 단계 이벤트·프로브 호출 순서, critic 통합, `disabled`, fail-soft, freshness 단계 |
...
| `test_freshness.py` | 3 | `fresh`/`stale`/`unverified` 판정 |
...
| `NARA_DOC_FRESHNESS_MODE=disabled` | 신선도 단계 제거, `freshness`는 빈 목록 |
...
| 신선도 검사 예외 | 경고 추가 + `freshness` 이벤트 `failed`, run은 `completed` |
| 매니페스트·인덱스 시각 부재 | `unverified` 보고 (신선도를 추정하지 않음) |
...
6. `NARA_INDEX_BUILT_AT`에 실제 인덱스 빌드 시각을 공급한다. 기본값이 빈 값이라
   지금은 신선도 판정이 항상 `unverified`로 끝난다 — Nara Search가 빌드 시각을
   노출하게 하거나 인덱스 빌드 절차에서 이 값을 채워야 실효가 생긴다.
7. **판단 이관 결정** — §6.2 Runs API 클라이언트를 구현할지, 아니면 프로브를
   다중 턴 세션으로 확장할지 정한다. 이 결정 없이는 §8.2 목표 계약과
   단계 5 평가가 진행 의미를 갖지 못한다.
8. 판단 이관 후: 목표 JSON 계약 도입 → critic 검사 확장 → shadow 평가 →
   `enabled` 승격 여부 결정 → 그 다음에만 승인형 memory·skill 실험.

남은 확장 후보(법령 근거 등)의 우선순위는
[`agent_expansion_exploration.md`](agent_expansion_exploration.md) §9를 따른다.
후보 A(계획 검증)·C(문서 신선도)·D(flow 내보내기)는 구현 완료다.
...
- Nara MCP 다섯 도구가 Hermes 세션 tool list에 등록되고, 그 외 도구가 없는가?
