...
def run_build(on_complete=None, device="cpu"):
    ...
    try:
        ...
        # 기존 manifest가 남은 채 산출물을 덮어쓰면 부분 빌드도 성공한 빌드처럼
        # 보인다. 첫 산출물을 쓰기 전에 이전 완료 기록을 무효화한다.
        config.BUILD_MANIFEST_PATH.unlink(missing_ok=True)
        ...
        manifest_tmp_path = config.BUILD_MANIFEST_PATH.with_suffix(".json.tmp")
        manifest_tmp_path.write_text(
            json.dumps({"built_at": built_at}, ensure_ascii=False),
            encoding="utf-8",
        )
        manifest_tmp_path.replace(config.BUILD_MANIFEST_PATH)
