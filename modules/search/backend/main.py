...
from .search.ranking import rank_candidates
...
def index_built_at() -> str:
    """마지막으로 성공한 인덱스 빌드의 완료 시각을 반환한다.

    빌드 manifest는 모든 산출물 기록과 완료 콜백이 성공한 뒤에만 생성된다.
    manifest가 없거나 손상됐으면 완료된 빌드 기록이 없는 것으로 본다.
    """
    try:
    ...
    return built_at if isinstance(built_at, str) else ""
...
def search(request: SearchRequest):
    ...
    ranked = rank_candidates(
        ...
        vector_search=retriever.search,
        lexical_search=lexical_retriever.search,
        detail_available=detail_provider.has_detail,
    )

    results = [_to_result(m) for m in ranked.results]
    return {
        "query": query,
        "results": results,
        "data_ready": detail_provider.data_ready(),
        "diagnostics": {
            "vector_enabled":     request.use_vector,
            "vector_candidates":  len(ranked.vector_candidates),
            "vector_error":       (retriever.last_error() or retriever.index_warning()) if not ranked.vector_candidates else "",
            ...
            "lexical_candidates": len(ranked.lexical_candidates),
            "lexical_source":     lexical_retriever.corpus_source(),
            "fusion":             ranked.fusion,
            "unavailable_candidates": ranked.unavailable_ids,
        },
    }
