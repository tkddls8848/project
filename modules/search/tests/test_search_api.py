...
def test_search_reports_data_not_ready_when_sources_are_missing(
...
def test_search_ranking_contract_with_conflicting_channels_coverage_and_stale_detail(
    app_client, monkeypatch
):
    """채널 불일치·커버리지 prior·상세 필터를 거친 최종 순서를 고정한다."""
    from backend import main

    regional = dict(
        _fake_search_results()[0],
        api_id="15000002",
        title="광주 병원 정보",
        provider="광주교통공사",
    )
    ...
    monkeypatch.setattr(
        main.retriever,
        "search",
        lambda query, top_k: [regional, nationwide, stale],
    )
    monkeypatch.setattr(main.retriever, "search_diagnostics", lambda: {})
    monkeypatch.setattr(
        main.lexical_retriever,
        "search",
        lambda query, top_k: [stale, regional, nationwide],
    )

    body = app_client.post(
        "/search",
        json={"query": "전국 병원 위치", "top_k": 2},
    ).json()

    assert [row["service_id"] for row in body["results"]] == [
        "openapi_new:15000001",
        "openapi_new:15000002",
    ]
    assert [row["score"] for row in body["results"]] == [0.035976, 0.032496]
    assert body["diagnostics"] == {
        "vector_enabled": True,
        "vector_candidates": 3,
        "vector_error": "",
        "vector_search": {},
        "vector_rrf_weight": 0.9,
        "lexical_rrf_weight": 1.1,
        "coverage_prior_bonus": 0.004,
        "lexical_candidates": 3,
        "lexical_source": "apidata_scan",
        "fusion": "rrf",
        "unavailable_candidates": ["openapi_new:15999999"],
    }
...
def test_health_does_not_report_build_time_without_manifest(app_client):
    from backend.core import config

    config.FAISS_INDEX_PATH.parent.mkdir(parents=True, exist_ok=True)
    config.FAISS_INDEX_PATH.write_bytes(b"stub index")

    assert app_client.get("/health").json()["index_built_at"] == ""
