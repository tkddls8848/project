...
- 한 번에 하나의 크롤만 허용한다. 목록 CSV와 `api_storage`가 공유 자원이다.

## 산출물 경로

크롤링 결과는 저장소 공통 데이터 루트 `../../api_storage/`에 저장된다 (run 폴더 없음).

- `../../api_storage/openapi_new/{api_id}.json` — OpenAPI(신형) 문서
- `../../api_storage/openapi_old/{api_id}.json` — OpenAPI(구형 HTML) 문서
- `../../api_storage/openapi_link/{api_id}.json` — LINK형 문서
- `../../api_storage/fileData/{api_id}.json`, `../../api_storage/standard/{api_id}.json`
- `../../api_storage/manifests/{run_id}.json` — 실행별 manifest (수집 파일 목록·체크섬)
- `../../api_storage/manifests/{run_id}_{type}_summary.json` — 실행별 요약
...
| 플래그 | 하는 일 | 산출물 |
| --- | --- | --- |
| `--deep` | fileData를 Range 샘플링으로 받아 스키마·품질·주소 리포트를 전부 생성 | `api_storage/reports/{run_id}_{file_schemas,quality,address_geo}.json` |
| `--full-download` | `--deep`을 샘플링 대신 전량 다운로드로 (포털 부하 큼) | 〃 + 받은 파일 `api_storage/files/{run_id}/` |
...
`--deep`은 `api_storage/fileData/`를, `--harvest`는 `api_storage/openapi_link/`를
