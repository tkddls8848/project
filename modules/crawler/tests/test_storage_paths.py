...
def test_storage_root_is_sibling_api_storage(tmp_path):
    # 마커가 없는 트리에서는 예전 규약(모듈이 루트의 직계 자식)으로 폴백한다.
    manager = CrawlRunManager(tmp_path / "nara_crawler")
    assert manager.storage_dir == tmp_path / "api_storage"
    assert manager.manifests_dir == tmp_path / "api_storage" / "manifests"
...
def test_raw_output_dir_is_flat_type_dir(tmp_path):
    ...
    assert manager.get_raw_output_dir("fileData") == tmp_path / "api_storage" / "fileData"
    assert manager.get_raw_output_dir("standard") == tmp_path / "api_storage" / "standard"
...
def test_save_manifest_goes_to_manifests_dir(tmp_path):
    ...
    assert path == tmp_path / "api_storage" / "manifests" / f"{run_id}.json"
...
def test_non_openapi_saves_flat_in_given_dir(tmp_path):
    ...
    type_dir = tmp_path / "api_storage" / "fileData"
