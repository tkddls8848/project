...
from backend.metadata_service import RANGE_TYPES
from backend.storage_service import DOCUMENT_DIRS
...
def test_crawl_type_contracts_stay_in_sync(monkeypatch, capsys):
    """결과 유형을 추가할 때 CLI·저장소·UI 중 한 곳만 빠지는 회귀를 막는다."""
    monkeypatch.setattr(sys, "argv", ["main.py", "--help"])
    with pytest.raises(SystemExit) as exc_info:
        asyncio.run(crawler_main.main())
    assert exc_info.value.code == 0
    help_text = capsys.readouterr().out
    cli_choices = tuple(re.search(r"\{([^}]+)\}", help_text).group(1).split(","))

    html = (Path(crawler_main.BASE_DIR) / "frontend" / "index.html").read_text(encoding="utf-8")
    select = re.search(r'<select id="typeInput">(.*?)</select>', html, re.DOTALL).group(1)
    frontend_types = tuple(re.findall(r'<option value="([^"]+)">', select))
    if not frontend_types:
        app_js = (Path(crawler_main.BASE_DIR) / "frontend" / "app.js").read_text(encoding="utf-8")
        assert "health.ui_data_types" in app_js
        frontend_types = tuple(asyncio.run(backend_main.health())["ui_data_types"])

    full_types = []

    async def fake_crawl_single_type(**kwargs):
        full_types.append(kwargs["data_type"])
        return None

    async def fake_depth_stages(*_args, **_kwargs):
        return {}

    monkeypatch.setattr(crawler_main, "find_latest_metadata_csv", lambda *_args: "metadata.csv")
    monkeypatch.setattr(crawler_main, "get_csv_range", lambda *_args: (1, 1))
    monkeypatch.setattr(crawler_main, "crawl_single_type", fake_crawl_single_type)
    monkeypatch.setattr(crawler_main, "run_depth_stages", fake_depth_stages)
    monkeypatch.setattr(crawler_main.CrawlRunManager, "save_manifest", lambda *_args: None)
    monkeypatch.setattr(sys, "argv", ["main.py", "--full", "--skip-update"])
    asyncio.run(crawler_main.main())

    expected_types = set(DATA_TYPES)
    assert cli_choices == tuple(DATA_TYPES)
    assert crawler_main.CSV_PREFIX_MAP == {
    ...
    assert set(crawler_main.CRAWLER_CLASSES) == expected_types
    assert tuple(DOCUMENT_DIRS) == (
        "openapi_new",
        "openapi_old",
        "openapi_link",
        "fileData",
    ...
    assert tuple(RANGE_TYPES) == ("openapi", "fileData", "standard")
    assert frontend_types == (
        "openapi",
        "fileData",
        "standard",
        "openapi_new",
    ...
    assert full_types == list(RANGE_TYPES)
