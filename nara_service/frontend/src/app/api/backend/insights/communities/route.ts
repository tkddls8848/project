...
// GET /api/backend/insights/communities - 커뮤니티 탐지
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const minSize = searchParams.get('min_size') || '3';

    const response = await fetch(
      `${BACKEND_URL}/insights/communities?min_size=${minSize}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Communities Detection API Error:', error);
    return NextResponse.json(
