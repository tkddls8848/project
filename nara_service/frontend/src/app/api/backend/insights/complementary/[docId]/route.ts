...
// GET /api/backend/insights/complementary/[docId] - 보완 데이터 추천
export async function GET(
  ...
  try {
    ...
    const response = await fetch(
      `${BACKEND_URL}/insights/complementary/${encodeURIComponent(docId)}?limit=${limit}`,
      {
        method: 'GET',
        headers: {
    ...
    console.error('Complementary Data API Error:', error);
    return NextResponse.json(
